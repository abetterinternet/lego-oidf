...
func NewEntity(identifier string) (Entity, error) {
	...
	if err != nil {
		return Entity{}, fmt.Errorf("failed to parse identifier '%s' as URL: %w", identifier, err)
	}

	if url.Scheme != "https" {
		return Entity{}, fmt.Errorf("Invalid OpenID Federation identifier '%s': does not use https scheme", identifier)
	}
...
func (e *Entity) ServeEntityConfiguration() error {
	...
	var err error
	e.listener, err = net.Listen("tcp", net.JoinHostPort("", e.identifier.Port()))
	if err != nil {
		return fmt.Errorf("could not start HTTP server for OIDF EC: %w", err)
	}

	log.Printf("listening on %s", net.JoinHostPort("", e.identifier.Port()))
	...
	go func() {
		...
		mux.HandleFunc(EntityConfigurationPath, func(w http.ResponseWriter, r *http.Request) {
			...
			log.Print("served OpenID Federation Entity Configuration")
		})
...
func (e *Entity) SignChallenge(token string) (*jose.JSONWebSignature, error) {
	challengeSigner, err := jose.NewSigner(
		jose.SigningKey{
			...
			// wait, key can be JSONWebKey. can I leave Algorithm to the 0 value and get the right
			// thing to happen?
			Algorithm: jose.RS256,
